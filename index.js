...
app.get('/api/config-status', (req, res) => {
  res.json({
    twilio:   !!(cfg.twilioSid && cfg.twilioToken && cfg.twilioPhone),
...
app.post('/api/config', (req, res) => {
  ...
  if (twilioSid)    cfg.twilioSid    = twilioSid;
  if (twilioToken)  cfg.twilioToken  = twilioToken;
  if (twilioPhone)  cfg.twilioPhone  = twilioPhone;
  if (anthropicKey) cfg.anthropicKey = anthropicKey;
...
app.post('/api/intent', async (req, res) => {
  ...
  if (!cfg.anthropicKey) return res.status(400).json({ error: 'Anthropic API 키를 먼저 설정해주세요.' });
...
app.post('/api/call/start', async (req, res) => {
  ...
  if (!cfg.twilioSid) return res.status(400).json({ error: 'Twilio 설정이 필요합니다.' });
  ...
  activeCalls.set(callId, {
    sessionId, institution, service,
    dtmfSequence: dtmfSequence||[], dtmfStep:0,
    phase:'dialing', log:[], startTime:Date.now()
  });
  ...
  try {
    ...
    const call = await client.calls.create({
      ...
      url: `${cfg.baseUrl}/twilio/voice?callId=${callId}`,
      statusCallback: `${cfg.baseUrl}/twilio/status?callId=${callId}`,
...
async function arsDecide(arsText, institution, service, dtmfSeq, step) {
  ...
  if (!cfg.anthropicKey) return { action:'agent', reason:'API 키 없음' };
  try {
    const claude = new Anthropic({ apiKey: cfg.anthropicKey });
    const msg = await claude.messages.create({
...
app.post('/twilio/voice', async (req, res) => {
  ...
  const g = twiml.gather({
    input:'speech dtmf', language:'ko-KR', speechTimeout:'auto',
    timeout:8, action:`${cfg.baseUrl}/twilio/gather?callId=${callId}`,
    method:'POST', numDigits:1
  });
  g.pause({ length:2 });
  twiml.redirect(`${cfg.baseUrl}/twilio/voice?callId=${callId}`);
...
app.post('/twilio/gather', async (req, res) => {
  ...
  if (agentKw.some(k => speech.includes(k))) {
    ...
    const g = twiml.gather({
      input:'speech', language:'ko-KR', speechTimeout:'auto',
      timeout:30, action:`${cfg.baseUrl}/twilio/agent?callId=${callId}`, method:'POST'
    });
    g.pause({ length:1 });
    ...
    if (decision.action === 'press') {
      ...
      g.pause({ length:2 });
      twiml.redirect(`${cfg.baseUrl}/twilio/gather?callId=${callId}`);
      ...
      const g = twiml.gather({
        input:'speech', language:'ko-KR', speechTimeout:'auto',
        timeout:60, action:`${cfg.baseUrl}/twilio/agent?callId=${callId}`, method:'POST'
      });
      g.pause({ length:1 });
    } else {
      const g = twiml.gather({
        input:'speech dtmf', language:'ko-KR', speechTimeout:'auto',
        timeout:10, action:`${cfg.baseUrl}/twilio/gather?callId=${callId}`,
        method:'POST', numDigits:1
      });
      g.pause({ length:3 });
      twiml.redirect(`${cfg.baseUrl}/twilio/gather?callId=${callId}`);
...
app.post('/twilio/agent', async (req, res) => {
  ...
  if (greetKw.some(k => speech.includes(k)) || s.phase === 'need_agent') {
    ...
    log(callId, 'agent_connected', '🔔 상담원 연결됨!', { agentSpeech: speech });
    twiml.say({ language:'ko-KR', voice:'Polly.Seoyeon' }, '잠시만 기다려 주세요. 고객을 연결해 드리겠습니다.');
    twiml.pause({ length:10 });
    const g = twiml.gather({
      input:'speech', language:'ko-KR', timeout:120,
      action:`${cfg.baseUrl}/twilio/agent?callId=${callId}`, method:'POST'
    });
    g.pause({ length:5 });
  } else {
    const g = twiml.gather({
